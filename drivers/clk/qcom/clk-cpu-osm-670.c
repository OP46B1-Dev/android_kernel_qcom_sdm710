...
osm_set_index(struct clk_osm *c, unsigned int index)
{
	...
	rounded_rate = clk_round_rate(c->hw.clk, rate);
	if (rounded_rate < 0)
		return rounded_rate;

	return clk_set_rate(c->hw.clk, rounded_rate);
}
...
static int clk_cpu_osm_driver_probe(struct platform_device *pdev)
{
	...
	struct clk *enabled_clks[ARRAY_SIZE(osm_qcom_clk_hws)];
	static const unsigned int l3_vote_clks[] = {
		L3_CLUSTER0_VOTE_CLK, L3_CLUSTER1_VOTE_CLK,
		L3_MISC_VOTE_CLK, L3_GPU_VOTE_CLK,
	};
	...
	for (i = 0; i < ARRAY_SIZE(l3_vote_clks); i++) {
		clk = clk_data->clks[l3_vote_clks[i]];
		rc = clk_prepare_enable(clk);
		if (rc)
			goto online_err;
		enabled_clks[num_enabled++] = clk;
	}
...
provider_err:
	while (num_enabled)
		clk_disable_unprepare(enabled_clks[--num_enabled]);
...
static struct platform_driver clk_cpu_osm_driver = {
	...
	.driver = {
		.name = "clk-cpu-osm-sdm670",
		.of_match_table = match_table,
		.owner = THIS_MODULE,
	},
};

static int __init clk_cpu_osm_sdm670_init(void)
{
	return platform_driver_register(&clk_cpu_osm_driver);
}
subsys_initcall(clk_cpu_osm_sdm670_init);

static void __exit clk_cpu_osm_sdm670_exit(void)
{
	platform_driver_unregister(&clk_cpu_osm_driver);
}
module_exit(clk_cpu_osm_sdm670_exit);

MODULE_DESCRIPTION("QTI SDM670 CPU clock driver for OSM");
